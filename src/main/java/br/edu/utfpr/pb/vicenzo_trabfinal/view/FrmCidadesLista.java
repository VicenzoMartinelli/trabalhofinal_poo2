...
import br.edu.utfpr.pb.vicenzo_trabfinal.tableModel.CidadeTableModel;
import javax.swing.JOptionPane;

public class FrmCidadesLista extends javax.swing.JInternalFrame {

    private CidadeTableModel cidadeTableModel;
    private CidadeController controller;

    public FrmCidadesLista() {
        initComponents();
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        FrmCidadeForm form = new FrmCidadeForm(null, true);
    ...
    private void btnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEditActionPerformed
        try {
            if (tblDados.getSelectedRow() >= 0) {
                Long id = Long.parseLong(cidadeTableModel.getValueAt(tblDados.getSelectedRow(), 0).toString());

                FrmCidadeForm form = new FrmCidadeForm(null, true);
                form.setLocationRelativeTo(null);
                form.carregarCidade(id);
                form.setVisible(true);
        ...
        try {
            if (tblDados.getSelectedRow() >= 0) {
                ...
                if(JOptionPane.showConfirmDialog(this, "Deseja realmente excluir?", "Atenção", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION)
                {
                    ...
                    cidadeTableModel.removeRow(tblDados.getSelectedRow());
                }
            } else {
                JOptionPane.showMessageDialog(this, "Selecione um registro!", "Atenção!!", JOptionPane.INFORMATION_MESSAGE);
            }
    ...
    private void loadData() {
        this.cidadeTableModel = new CidadeTableModel(controller.listar());
